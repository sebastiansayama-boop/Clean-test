...
class ControlledRouter:
    ...
    async def submit(self, request: Request):
        ...
        op = Operation(
            operation_id="op_" + uuid4().hex[:12],
            request_id=request.request_id,
            action=analysis.requested_action,
            resource="order:" + (analysis.order_id or "unknown"),
            status=OperationStatus.ANALYZING,
        )
        self._evidence(op, "REQUEST", "request", request.model_dump())
        self._evidence(op, "ANALYSIS", "agent", analysis.model_dump())
        ...
        op.status = OperationStatus.VALIDATING
        self._save(op)
        if not payment:
            return self._finish(op, OperationStatus.FAILED, "Order/payment not found.")
        ...
        op.authorization = "allowed" if decision.allowed else "denied"
        op.arguments = {
            "amount": amount,
            "payment_id": payment.payment_id,
            "reason": "Customer request",
        }
        self._evidence(op, "POLICY_DECISION", "policy", decision.model_dump())
        self._save(op)
        if not decision.allowed:
            return self._finish(op, OperationStatus.FAILED, decision.reason)
    ...
    def approve(self, oid, approve):
        ...
        if claimed is None:
            raise KeyError(oid)
        if claimed is False:
            raise ValueError("Operation is not pending approval.")
        op = claimed
        ...
        if not approve:
            return self._finish(
                ...
                "Human approval rejected the external effect.",
            )
    ...
    def reconcile(self, oid):
        ...
        if op.status != OperationStatus.UNKNOWN:
            return op
        op.status = OperationStatus.RECONCILING
        self._save(op)
        refund = self.provider.get_refund(
            op.arguments["payment_id"], op.arguments["amount"]
        )
        self._evidence(
            op,
            "RECONCILIATION",
            "provider",
            {"found": bool(refund), "refund": refund},
        )
        if refund:
            op.external_reference = refund["refund_id"]
            op.result = refund
            return self._finish(
                op,
                OperationStatus.COMPLETED,
                "Reconciliation confirmed the external effect.",
            )
        return self._finish(
            op,
            OperationStatus.UNKNOWN,
            "External outcome remains unresolved.",
        )
    ...
    def _execute(self, op, payment_id, amount):
        ...
        op.external_reference = result["refund_id"]
        op.result = result
        ...
        self._evidence(
            ...
            "VERIFICATION",
            "provider",
            {"verified": bool(verified), "refund": verified},
        )
        if not verified:
            return self._finish(
                op,
                OperationStatus.UNKNOWN,
                "Verification did not confirm the effect.",
            )
    ...
    def _finish(self, op, status, message):
        op.status = status
        op.result = {**(op.result or {}), "message": message}
        self._save(op)
        return op
